...
\end{table}"""
    ...
    # Leggi il file tex
    tex_path = os.path.join("docs", "la_sorte_del_giocatore.tex")
    ...
    res1 = subprocess.run(["pdflatex", "-interaction=nonstopmode", "-output-directory=outputs", tex_path], capture_output=True, text=True)
    ...
    res2 = subprocess.run(["pdflatex", "-interaction=nonstopmode", "-output-directory=outputs", tex_path], capture_output=True, text=True)
    if res2.returncode != 0:
        print("Errore nella seconda compilazione LaTeX:")
        print(res2.stdout[:1000])
        sys.exit(1)

    print("Successo! PDF generato correttamente: outputs/la_sorte_del_giocatore.pdf")
